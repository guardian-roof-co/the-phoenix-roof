...
export const UserProfile: React.FC<UserProfileProps> = ({ onNavigate }) => {
  ...
  const shareUrl = 'https://roofbyphoenix.com'; // Simulated URL for demo
                ...
                <div className="space-y-4">
                  {appointments.map(appt => (
                    <div key={appt.id} className="flex items-center justify-between p-4 bg-gray-50 rounded-lg border-l-4 border-phoenix-500">
                      ...
                      <span className={`px-3 py-1 rounded-full text-xs font-bold uppercase ${appt.status === 'confirmed' ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'
                        }`}>
                        {appt.status}
                      </span>
                    </div>
